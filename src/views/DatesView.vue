<template>
  <div>
    <h2>Upcoming Dates</h2>
    <div class="mt-10">
      <article
        class="text-black p-4 my-4 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">30 September, 2023</h3>
        <span class="text-sm text-gray-600 mb-2 block font-bold"
          >King’s Place, London, United Kingdom.</span
        >
        <span class="text-base mb-2 block sm:text-center"
          >World premiere of ‘Whirling Dervishes’ (2022). Shiva Feshareki (Ambisonic Turntables).
          Zubin Kanga (Piano)</span
        >
        <a
          href="https://www.kingsplace.co.uk/whats-on/contemporary/zubin-kanga-with-shiva-feshareki/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">2 Dec, 2023</h3>
        <span class="text-sm text-gray-600 mb-2 block font-bold sm:text-center"
          >King’s Place, London, United Kingdom.</span
        >
        <span class="text-base mb-2 block sm:text-center"
          >World premiere of ‘Seismic Wave Orchestra’ (2020). Shiva Feshareki (Composition &
          Ambisonic Turntables). Sean Shibe (Electric Guitar).</span
        >
        <a
          href="https://www.kingsplace.co.uk/whats-on/contemporary/zubin-kanga-with-shiva-feshareki/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700"
          >TICKETS AND MORE INFO</a
        >
      </article>
    </div>
    <h2>Previous Dates</h2>
    <div class="mt-10">
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">11 March, 2023</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Iklectik, London, United Kingdom.</span>
        <span class="text-base mb-2 block sm:text-center"
          >Live Ambisonic Turntable Performance + Rrose (live)</span
        >
        <a
          href="https://ra.co/events/1659976"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-4 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">16 March, 2023</h3>
        <span class="text-sm text-gray-600 mb-2 block font-bold sm:text-center"
          >Royal Welsh College of Music & Drama, Cardiff, Wales.</span
        >
        <span class="text-base mb-2 block sm:text-center"
          >‘TRANSFIGURE’ (2022) acoustic premiere performed by The Hermes Ensemble.</span
        >
        <a
          href="https://www.rwcmd.ac.uk/events/2023-03/hermes-experiment"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">8 Feb, 2023</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Iklectik, London, United Kingdom.</span>
        <a
          href="https://www.uniarts.fi/tapahtumat/marianna-henriksson-keskisavel/?utm_source=facebook&utm_medium=event&utm_campaign=keskisavel-info"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">26 Jan 2023</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Milton Court, Barbican</span>
        <a
          href="https://www.uniarts.fi/tapahtumat/marianna-henriksson-keskisavel/?utm_source=facebook&utm_medium=event&utm_campaign=keskisavel-info"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">3 Dec 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold"
          >Ears to the Ground Festival, De Bijloke</span
        >
        <a
          href="https://www.bijloke.be/pQWVpSL/ear-to-the-ground-festival"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">25 Nov 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">The Sheldonian</span>
        <a
          href="https://www.torch.ox.ac.uk/event/sampling-the-city-shiva-feshareki-live-at-the-sheldonian"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">9 Oct 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Goat Song Project, Dom im Berg</span>
        <a
          href="https://musikprotokoll.orf.at/en/2022/project/goat-song-project"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">7 Oct 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Southbank Centre</span>
        <a
          href="https://www.southbankcentre.co.uk/whats-on/classical-music/xenakis-100"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">7 Oct 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Helmut List Halle</span>
        <a
          href="https://musikprotokoll.orf.at/en/2022/project/rso-wien-2022"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">30 Apr 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Spor Festival</span>
        <a
          href="https://www.sporfestival.dk/ukshiva"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">11 Mar 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Centre for the Arts</span>
        <a
          href="https://www.hellerau.org/en/event/torus-of-revolution/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">11 Mar 2022</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Stoller Hall</span>
        <a
          href="https://stollerhall.com/whats-on/jess-gillam-with-manchester-camerata/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">12 Oct 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Netherlands Tour</span>
        <a
          href="https://www.nederlandskamerkoor.nl/en/concertenreeks/ester/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">28 Aug 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold"
          >Alternative Stage, Greek National Opera</span
        >
        <a
          href="https://www.snfnostos.org/en/2021/288/ergon-ensemble-and-shiva-feshareki/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">19 Aug 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">BBC Proms, Royal Albert Hall</span>
        <a
          href="https://www.bbc.co.uk/events/engrn3?gclid=Cj0KCQjw0K-HBhDDARIsAFJ6UGiPJBiLqipgEJTLGhkdsw9_wm4-S0zAc8gWanh5mf89LNayb5oklFQaAhI1EALw_wcB&gclsrc=aw.ds"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">31 Jul 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Worcester Cathedral</span>
        <a
          href="https://3choirs.org/whats-on/national-youth-choir-of-great-britain/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">15 May 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">KunstFestSpieleb</span>
        <a
          href="https://kunstfestspiele.de/en/festival/archive/archive-2021.html"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">23 Mar 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Stoller Hall</span>
        <a
          href="https://stollerhall.com/blog/bbc-radio-3-manchester-week-at-the-stoller-hall-2/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">17 Feb 2021</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">Alternative Space LOOP Gallery</span>
        <a
          href="http://altspaceloop.chttp//www.refugia.kr/index_en.htmlom/exhibitions/refugia-2021"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">16 Nov 2020</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">BEETHOVEN REMIXED’</span>
        <a
          href="https://www.bbc.co.uk/programmes/m000q38j"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">22 Nov 2020</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold"
          >Huddersfield Contemporary Music Festival</span
        >
        <a
          href="https://hcmf.co.uk/"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
      <article
        class="text-black p-4 my-8 border border-black flex-col items-center justify-center w-full inline-flex"
      >
        <h3 class="text-2xl font-bold mb-2">31 Jul 2020</h3>
        <span class="text-sm text-gray-600 mb-2 font-bold">The Design Museum</span>
        <a
          href="https://designmuseum.org/exhibitions/electronic-from-kraftwerk-to-the-chemical-brothers/electronic-catch-up"
          class="bg-black text-base text-white py-2 px-4 inline-block hover:bg-gray-700 sm:text-center"
          >TICKETS AND MORE INFO</a
        >
      </article>
    </div>
  </div>
  <TheScrollToTop />
</template>

<script setup>
import TheScrollToTop from '@/components/TheScrollToTop.vue'
</script>

<style lang="scss" scoped></style>
